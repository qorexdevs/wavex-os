...
- **Mailman list bounces too** - the bounce check now flags the `<list>-bounces@` return path Mailman routes list bounces to, and its VERP form `<list>-bounces+token@`, which the earlier prefix-only checks missed since the bounce tag sits after the list name. A `-bounces` infix flags while `<list>-owner@` and `<list>-request@` stay repliable.
- **SRS forwarding addresses** — the no-reply guard now flags `srs0=`/`srs1=` senders, the Sender Rewriting Scheme return paths a forwarder puts on relayed mail. They route bounces back to the forwarder, not a person, so a drafted reply is wasted; the leading `srs0=`/`srs1=` tag matches while `srsly@` and a plain `srs@` stay human.
...
- **Mailman sender matching stays exact** — a local part such as `team-bouncesmith@` no longer trips the Mailman `<list>-bounces@` rule. The no-reply cue now accepts only the complete `-bounces` segment and its `+token` form.
