...
- **VERP bounce addresses too** — the bounce check now also catches the per-recipient return paths ESPs actually use, `bounces+SRS=token@` and `bounce-12345@`, not just the bare `bounce@`/`bounces@`. A numbered bounce or a `bounces*` prefix flags; `bouncer@` stays human.
