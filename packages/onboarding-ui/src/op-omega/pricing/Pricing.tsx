 ...
 *
 *  Both paths call onContinue(); the parent (OmegaOnboarding) then opens
...
export function Pricing({ companyId, onContinue }: PricingProps) {
  ...
  const q = useQuery({
    queryKey: ["tiers"],
    queryFn: () => opOmegaOnboardingApi.listTiers(),
  });
  ...
  async function handleChoice(tierId: TierId, origin: "subscribe" | "skip"): Promise<void> {
    ...
    setError(null);
    try {
      await opOmegaOnboardingApi.subscribeTier({ orgId: companyId, tierId, origin });
