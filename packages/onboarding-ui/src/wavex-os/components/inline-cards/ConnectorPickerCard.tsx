 ...
 *
 *  Renders the three buckets returned by /wavex-os/onboarding/connector-
...
import type { ConnectorManifest } from "@wavex-os/plugin-onboarding";
...
export function ConnectorPickerCard({ companyId, manifest, onConfirmed, onReRefined }: Props) {
  ...
  async function handleRefine(): Promise<void> {
    ...
    setError(null);
    try {
      const result = await wavexOsOnboardingApi.generateConnector(companyId, false);
