...
export function RedundancyReview({ companyId, onChange }: Props) {
  ...
  const q = useQuery({
    ...
    queryFn: () => wavexOsOnboardingApi.getRedundancy(companyId),
  ...
  async function toggleMute(slot: string, currentlyMuted: boolean): Promise<void> {
    ...
    setError(null);
    try {
      if (currentlyMuted) await wavexOsOnboardingApi.unmuteSlot(companyId, slot);
      else await wavexOsOnboardingApi.muteSlot(companyId, slot);
