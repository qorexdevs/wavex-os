...
export interface AgentSwapPanelProps {
   ...
   *  base wavex-os roster). Removable via the panel. */
...
export function AgentSwapPanel({
  ...
  async function save(): Promise<void> {
    ...
    setError(null);
    try {
      const r = await wavexOsOnboardingApi.swapTemplate({
        companyId, slot,
  ...
  async function resetToDefault(): Promise<void> {
    ...
    setError(null);
    try {
      await wavexOsOnboardingApi.swapTemplate({ companyId, slot, templateId: null });
      onSwapped(null);
      onClose();
    } catch (e) {
      setError(e instanceof ApiError ? e.message : (e as Error).message);
  ...
  async function addAsNewAgent(): Promise<void> {
    ...
    setError(null);
    try {
      const r = await wavexOsOnboardingApi.addAgent({
  ...
  async function removeThisAddedAgent(): Promise<void> {
    ...
    setError(null);
    try {
      await wavexOsOnboardingApi.removeAddedAgent({ companyId, slot });
      onRemoved(slot);
      onClose();
    } catch (e) {
      setError(e instanceof ApiError ? e.message : (e as Error).message);
