 ...
 *  /wavex-os/onboarding/pillar/:n/suggest with the current companyId; the
...
import { wavexOsOnboardingApi, ApiError } from "./api";
...
export function usePillarSuggestion(pillar: SuggestablePillar, companyId: string | null | undefined): PillarSuggestion {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const r = await wavexOsOnboardingApi.pillarSuggest(pillar, companyId);
        if (cancelled) return;
        setState({
