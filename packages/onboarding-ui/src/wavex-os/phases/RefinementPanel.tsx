...
type AnalyzeChange = Awaited<ReturnType<typeof wavexOsOnboardingApi.analyzeRefinement>>["changes"][number];
...
export function RefinementPanel({ companyId, hasRefinementHistory, onManifestUpdated }: Props) {
  ...
  async function handleAnalyze(): Promise<void> {
    ...
    try {
      const r = await wavexOsOnboardingApi.analyzeRefinement({ companyId, operatorGuidance: guidance.trim() });
      setAnalyzeResult({
  ...
  async function handleRevert(): Promise<void> {
    ...
    try {
      ...
      const m = await wavexOsOnboardingApi.getInstanceManifest(companyId);
