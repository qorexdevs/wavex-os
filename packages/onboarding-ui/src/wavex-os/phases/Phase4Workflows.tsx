...
export function Phase4Workflows({ companyId, onComplete }: Props) {
  ...
  async function generate(skipInference = false): Promise<void> {
    ...
    setError(null);
    try {
      const r = await wavexOsOnboardingApi.generateWorkflow(companyId, { skipInference, bypassBudgetCheck: true });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const loaded = await wavexOsOnboardingApi.loadWorkflow(companyId);
        if (!alive) return;
