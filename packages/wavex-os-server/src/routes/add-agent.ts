...
interface ManifestWithAdditions extends CompanyManifest {
  ...
  /** Marker for the pre-finalize fallback path — set when we wrapped
   *  swarm_manifest.json because company.manifest.json doesn't exist yet.
   *  writeManifest persists additions to a sidecar in that case. */
  __from_swarm_manifest_only?: boolean;
}
...
/** Load the manifest the add/remove handlers operate on.
 *
 *  Reads company.manifest.json first (post-finalize). If it doesn't exist
 *  yet (the operator is still in Swarm Studio, pre-Imprint), falls back
 *  to swarm_manifest.json and wraps it in a minimal shell so the add-agent
 *  logic can run against the in-progress swarm. The fallback shell is
 *  marked with `__from_swarm_manifest_only: true` so writeManifest knows
 *  to persist additions back to swarm_manifest.json instead of overwriting
 *  the non-existent company.manifest.json with a bogus full shape.
 *
 *  This kills the misleading "manifest not found — finalize first" error
 *  the operator hit when trying to add a WhatsApp agent during Studio. */
async function loadManifest(companyId: string): Promise<ManifestWithAdditions | null> {
  ...
  } catch { /* fall through to swarm-manifest fallback */ }
  try {
    const path = join(getOnboardingDir(companyId), "swarm_manifest.json");
    const swarm = JSON.parse(await readFile(path, "utf8")) as ManifestWithAdditions["swarm_manifest"];
    // Minimal shell — enough fields that the add-agent route can validate
    // parent_slot and persist template_additions. writeManifest below
    // serializes additions to a sidecar file so we don't fabricate a bogus
    // company.manifest.json pre-finalize.
    // Cast via unknown — the partial shell intentionally omits fields
    // (pillar_responses, finalized_at, etc.) that only exist post-finalize.
    // The add-agent route never reads them; only swarm_manifest.agents +
    // template_additions are touched on this code path.
    return {
      schema_version: "1.0",
      generated_at: new Date().toISOString(),
      generated_by: "swarm-studio-pre-finalize",
      based_on: { swarm_manifest_hash: "pre-finalize" },
      swarm_manifest: swarm,
      __from_swarm_manifest_only: true,
    } as unknown as ManifestWithAdditions;
  } catch { return null; }
}

async function writeManifest(companyId: string, m: ManifestWithAdditions): Promise<void> {
  const dir = getOnboardingDir(companyId);
  // Pre-finalize: only the additions sidecar persists; company.manifest.json
  // gets generated by the finalize flow with the proper signed shape.
  if (m.__from_swarm_manifest_only) {
    await writeFile(
      join(dir, "template_additions.json"),
      JSON.stringify({ template_additions: m.template_additions ?? [] }, null, 2),
      "utf8",
    );
    return;
  }
