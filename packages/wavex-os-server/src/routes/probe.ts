...
import { probeClaudeCode } from "@wavex-os/plugin-onboarding";
...
export function registerProbeRoutes(app: FastifyInstance): void {
  app.get("/wavex-os/onboarding/claude-code-check", async (req, reply) => {
  ...
  app.get("/wavex-os/onboarding/loop-status", async (req, reply) => {
    const ar = authReq(req);
    try { assertBoard(ar); } catch (e) {
      if (e instanceof AuthError) return reply.status(e.statusCode).send({ error: e.message });
      throw e;
