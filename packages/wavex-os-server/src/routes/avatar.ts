 ...
 *    POST /wavex-os/onboarding/avatar                  → create
 *    POST /wavex-os/onboarding/avatar/:id/tools        → record mock connect
 *    POST /wavex-os/onboarding/avatar/:id/voice        → save samples + analyze
 *    GET  /wavex-os/onboarding/avatar/:id/suggestions  → rule-based automations
 *    POST /wavex-os/onboarding/avatar/:id/finalize     → enable picked automations
...
export function registerAvatarRoutes(app: FastifyInstance): void {
  ...
  app.post("/wavex-os/onboarding/avatar/parse", async (req, reply) => {
  ...
  // Create avatar
  app.post("/wavex-os/onboarding/avatar", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
  ...
  app.post<{ Params: { id: string } }>("/wavex-os/onboarding/avatar/:id/tools", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
  ...
  app.post<{ Params: { id: string; provider: string } }>(
    "/wavex-os/onboarding/avatar/:id/tools/:provider/meta",
    async (req, reply) => {
      if (!gateBoard(req, reply)) return;
      const id = req.params.id;
      const dir = avatarDir(id);
  ...
  app.post<{ Params: { id: string } }>("/wavex-os/onboarding/avatar/:id/voice", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
    const id = req.params.id;
    const dir = avatarDir(id);
    if (!existsSync(dir)) return reply.status(404).send({ error: "avatar not found" });
  ...
  app.post<{ Params: { id: string } }>("/wavex-os/onboarding/avatar/:id/trust", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
    const id = req.params.id;
    const dir = avatarDir(id);
    if (!existsSync(dir)) return reply.status(404).send({ error: "avatar not found" });
  ...
  app.get<{ Params: { id: string }; Querystring: { skipInference?: string } }>(
    "/wavex-os/onboarding/avatar/:id/suggestions",
    async (req, reply) => {
      if (!gateBoard(req, reply)) return;
      const id = req.params.id;
      const dir = avatarDir(id);
  ...
  app.post<{ Params: { id: string } }>("/wavex-os/onboarding/avatar/:id/finalize", async (req, reply) => {
    if (!gateBoard(req, reply)) return;
    const id = req.params.id;
    const dir = avatarDir(id);
    if (!existsSync(dir)) return reply.status(404).send({ error: "avatar not found" });
