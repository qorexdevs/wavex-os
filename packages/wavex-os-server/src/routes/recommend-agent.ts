/** POST /wavex-os/onboarding/recommend-agent
 *
...
async function loadRegistry(): Promise<RegistryTemplate[]> {
  ...
  // packages/wavex-os-server/src/routes → walk up to repo root
  let dir = here;
  for (let i = 0; i < 10; i++) {
...
export function registerRecommendAgentRoute(app: FastifyInstance): void {
  app.post("/wavex-os/onboarding/recommend-agent", async (req, reply) => {
