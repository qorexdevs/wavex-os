/** Smoke test: Fastify boots, op-omega routes register, status endpoint
...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), "op-omega-server-test-"));
...
describe("@wavex-os/op-omega-server", () => {
  ...
  it("status endpoint returns initialized pillar shape with dev-mode auth bypass", async () => {
    ...
    const r = await app.inject({
      method: "GET",
      url: "/op-omega/onboarding/status?companyId=test-co",
    });
  ...
  it("status endpoint without companyId returns 400", async () => {
    ...
    registerOpOmegaRoutes(app);
    await app.ready();
    const r = await app.inject({ method: "GET", url: "/op-omega/onboarding/status" });
  ...
  it("connector-recommendations returns a manifest even with empty pillars", async () => {
    ...
    const r = await app.inject({
      method: "GET",
      url: "/op-omega/onboarding/connector-recommendations?companyId=fresh-co",
    });
    expect(r.statusCode).toBe(200);
    const body = r.json();
    expect(body.ok).toBe(true);
  ...
  it("loop-status returns idle by default", async () => {
    ...
    registerOpOmegaRoutes(app);
    await app.ready();
    const r = await app.inject({ method: "GET", url: "/op-omega/onboarding/loop-status" });
    expect(r.statusCode).toBe(200);
    const body = r.json();
    expect(body.ok).toBe(true);
