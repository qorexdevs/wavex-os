...
| `platform-reliability-engineer` | SRE for the operator. Computes aggregate fleet metrics, watches per-customer cost spikes, probes operator-local launchd services (`/api/system/health`, mock-core, op-omega-server, inference-server). | Read aggregate fleet state. File P1 (fleet regressions) or SEV0 (operator-local outage) issues. Coalesces — one issue per check per cycle. | Admin CEO → operator Telegram (always, on SEV0). |
