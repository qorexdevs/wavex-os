...
export function CredentialConciergeStep({
  ...
  const refetch = () => {
    qc.invalidateQueries({ queryKey: ["op-omega", "credentials", companyId] });
  };
...
function CredentialCard({
  ...
  const paste = useMutation({
    ...
    mutationFn: () =>
      opOmegaOnboardingApi.credentialsPaste({ companyId, credentialKey, plaintext: draft }),
    onSuccess: (res) => {
      setResult(res);
      if (res.status === "valid") {
        setDraft("");
  ...
  const skip = useMutation({
    mutationFn: () =>
      opOmegaOnboardingApi.credentialsSkip({
        companyId,
        credentialKey,
...
function UnknownCredentialDialog({
  ...
  const submit = useMutation({
    mutationFn: () =>
      opOmegaOnboardingApi.credentialsUnknown({
        companyId,
        label,
        purpose,
        plaintext,
