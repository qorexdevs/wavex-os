...
export function Phase1Host({
  ...
  companyId: string;
  status: NonNullable<Awaited<ReturnType<typeof opOmegaOnboardingApi.status>>>;
  onComplete: () => void;
}) {
  ...
  const confirm = useMutation({
    mutationFn: opOmegaOnboardingApi.pillar1ConfirmInference,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["op-omega", "onboarding-status", companyId] });
    },
  });
