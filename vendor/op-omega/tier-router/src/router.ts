...
export async function route(
  ...
  const paperclipBaseUrl = options.paperclipBaseUrl ?? process.env.OP_OMEGA_PAPERCLIP_BASE_URL ?? "http://127.0.0.1:3102";
  ...
  if (decision.tier === 1) {
    try {
      const r = await invokeOllama({
        baseUrl: options.ollamaBaseUrl ?? process.env.OP_OMEGA_OLLAMA_BASE_URL ?? "http://127.0.0.1:11434",
        model: options.ollamaModel ?? process.env.OP_OMEGA_OLLAMA_MODEL ?? "llama3.2:3b",
  ...
  const t2 = await invokeClaudeCode({
    bin: options.claudeBin ?? process.env.OP_OMEGA_CLAUDE_BIN ?? "claude",
    prompt: request.prompt,
