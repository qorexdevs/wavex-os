...
import { route, fetchBudgetSnapshotWithRetry } from "@op-omega/plugin-tier-router";
...
export async function generateWorkflowManifest(
  ...
  if (!input.skipInference && !input.bypassBudgetCheck) {
    const baseUrl = input.paperclipBaseUrl ?? process.env.OP_OMEGA_PAPERCLIP_BASE_URL ?? "http://127.0.0.1:3102";
