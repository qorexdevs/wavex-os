...
- `pnpm test:differential-equation:live` · `OP_OMEGA_TEST_LIVE=1` variant (~60–90 min, ~150–200 T2 calls on 3-fixture subset)
...
`ui/src/pages/OmegaOnboarding.tsx`
...
Applied to: `OmegaOnboarding.tsx` (phase labels, CompanyView, WorkflowView, step indicators) + `SwarmOrgChart.tsx` (topology badges, bundle allocation section). `source: t2/fallback` code line replaced with "Shaped by your specific answers" / "Built from your pillar signals".
...
Server: `POST /op-omega/onboarding/generate/workflow` returns HTTP 424 with `{ error: "onboarding_halt", halt }` on halt.
...
- `ui/src/components/op-omega/onboarding/KPIVerification.tsx` — form rendering each KPI with inline edit. Verified fields get an emerald badge; AI estimates get an amber badge. Value input accepts various formats (e.g. `42` or `42%` or `$5,000`).
...
- Server endpoint: `POST /op-omega/onboarding/kpi-verify` — merges operator values into `pillar_3.kpi_snapshot_initial`, flips `ai_estimated` flag based on count of verified fields.
- Flow wiring: new `kpi_verify` phase inserted between `workflow` and `finalize` in `OmegaOnboarding.tsx` progression.
...
Server:
- `server/src/routes/op-omega-onboarding.ts`

UI:
- `ui/src/i18n/strategy-names.ts` · `mc-context.ts` · `phase-labels.ts` · `kpi-names.ts` (all new)
- `ui/src/components/op-omega/onboarding/KPIVerification.tsx` (new)
- `ui/src/components/op-omega/onboarding/SwarmOrgChart.tsx`
- `ui/src/pages/OmegaOnboarding.tsx` (largest surface — pillars, phase components, HaltScreen, WorkflowPhase, KPIVerifyPhase, transition hint store)
- `ui/src/api/opOmegaOnboarding.ts`
