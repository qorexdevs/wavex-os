...
import { wavexOsOnboardingApi, type OnboardingStatus, type Pillar1Response } from "../../../api/wavexOsOnboarding";
...
export function MaterializePhase({
  ...
  const [result, setResult] = useState<Awaited<ReturnType<typeof wavexOsOnboardingApi.materialize>> | null>(null);
  const [err, setErr] = useState<string | null>(null);
  const run = useMutation({
    mutationFn: () => wavexOsOnboardingApi.materialize({ companyId }),
    onSuccess: (r) => {
      setResult(r);
      setErr(null);
    },
  ...
  const loopStatus = useQuery({
    queryKey: ["wavex-os", "loop-status", companyId, result?.created.length ?? 0],
    queryFn: () => wavexOsOnboardingApi.loopStatus(companyId),
