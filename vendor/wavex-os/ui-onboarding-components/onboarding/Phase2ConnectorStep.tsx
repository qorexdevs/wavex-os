...
export function Phase2ConnectorStep({
  ...
  const declinedStorageKey = `wavex-os.declined-connectors.${companyId}`;
  ...
  const recs = useQuery({
    queryKey: ["wavex-os", "connector-recommendations", companyId],
    queryFn: () => wavexOsOnboardingApi.connectorRecommendations(companyId),
...
function ToolkitCard({
  ...
  const initiate = useMutation({
    mutationFn: () =>
      wavexOsOnboardingApi.initiateConnector({
        companyId,
