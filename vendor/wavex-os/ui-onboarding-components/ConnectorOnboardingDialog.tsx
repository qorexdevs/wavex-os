...
export function ConnectorOnboardingDialog({ open, onOpenChange, companyId }: Props) {
  ...
  const connectorsQ = useQuery({
    queryKey: ["wavex-os", "connectors", companyId],
    queryFn: () => wavexOsApi.listConnectors(companyId),
    enabled: open && !!companyId,
  });
  const oauthAppsQ = useQuery({
    queryKey: ["wavex-os", "oauth-apps", companyId],
    queryFn: () => wavexOsApi.listOAuthApps(companyId),
    enabled: open && !!companyId,
  });

  const pasteMut = useMutation({
    mutationFn: (args: { id: string; secrets: Record<string, string> }) =>
      wavexOsApi.onboardConnector(args.id, { companyId, secrets: args.secrets }),
    onSuccess: async (result) => {
      ...
      qc.invalidateQueries({ queryKey: ["wavex-os", "connectors", companyId] });

      if (result.followup === "supabase-pick-project") {
        try {
          const projects = await wavexOsApi.supabaseListProjects(companyId);
          setPhase((prev) =>
  ...
  const saveAppMut = useMutation({
    mutationFn: (args: { provider: "github" | "slack" | "supabase"; clientId: string; clientSecret: string }) =>
      wavexOsApi.saveOAuthApp(args.provider, {
        companyId,
        clientId: args.clientId,
        clientSecret: args.clientSecret,
      }),
    onSuccess: (_result, vars) => {
      qc.invalidateQueries({ queryKey: ["wavex-os", "oauth-apps", companyId] });
      setAppClientId("");
      setAppClientSecret("");
      setErrorMsg(null);
  ...
  const resetAppMut = useMutation({
    mutationFn: (provider: "github" | "slack" | "supabase") =>
      wavexOsApi.resetOAuthApp(provider, companyId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["wavex-os", "oauth-apps", companyId] });
    },
  });
  ...
  const supabasePickMut = useMutation({
    mutationFn: (args: { projectRef: string }) =>
      wavexOsApi.supabaseSelectProject({ companyId, projectRef: args.projectRef }),
    onSuccess: (result) => {
      qc.invalidateQueries({ queryKey: ["agents", companyId] });
      qc.invalidateQueries({ queryKey: ["secrets", companyId] });
      setPhase((prev) =>
            ...
            <div className="flex items-center justify-between pl-8">
              <button
                ...
                onClick={async () => {
                  ...
                  const app = (await wavexOsApi.listOAuthApps(companyId)).find((a) => a.provider === phase.spec.oauthProvider);
