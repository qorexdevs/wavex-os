 ...
 * `components/wavex-os/onboarding/`; this file routes between them.
 */
...
import { SwarmOrgChart } from "../components/wavex-os/onboarding/SwarmOrgChart";
import { Phase1Host } from "../components/wavex-os/onboarding/phase1-host";
import { GenerateManifestPhase } from "../components/wavex-os/onboarding/generate-manifest-phase";
import { ConnectorView } from "../components/wavex-os/onboarding/connector-view";
import { CompanyView } from "../components/wavex-os/onboarding/company-view";
import { WorkflowPhase } from "../components/wavex-os/onboarding/workflow-phase";
import { KPIVerifyPhase } from "../components/wavex-os/onboarding/kpi-verify-phase";
import { MaterializePhase } from "../components/wavex-os/onboarding/materialize-phase";
import { StepDot } from "../components/wavex-os/onboarding/primitives";
import { WelcomeScreen } from "../components/wavex-os/onboarding/welcome-screen";
import { Phase2ConnectorStep } from "../components/wavex-os/onboarding/Phase2ConnectorStep";
import { CredentialConciergeStep } from "../components/wavex-os/onboarding/CredentialConciergeStep";
import { describePhase, formatTimeRemaining } from "../components/wavex-os/onboarding/progress-helpers";
...
export function WavexOsOnboarding() {
  ...
  const handleStartFresh = async () => {
    ...
    try {
      ...
      qc.removeQueries({ queryKey: ["wavex-os", "onboarding-status"] });
  ...
  const { data: status, isLoading, isFetching } = useQuery({
    queryKey: ["wavex-os", "onboarding-status", selectedCompanyId],
    queryFn: () => wavexOsOnboardingApi.status(selectedCompanyId!),
    ...
    >
      ...
      {(phase === 1 || phase === 2 || phase === 3 || phase === 4) && status && (
        <Phase1Host
          ...
          status={status}
          onComplete={() => {
            qc.invalidateQueries({ queryKey: ["wavex-os", "onboarding-status", selectedCompanyId] });
      ...
      {phase === "connector_pick" && (
        <Phase2ConnectorStep
          ...
          onSessionExpired={() => {
            ...
            qc.invalidateQueries({ queryKey: ["wavex-os", "credentials", selectedCompanyId] });
      ...
      {phase === "connector" && (
        <GenerateManifestPhase
          ...
          generate={() => wavexOsOnboardingApi.generateConnector(selectedCompanyId, skipInference)}
      ...
      {phase === "swarm" && (
        <GenerateManifestPhase
          ...
          generate={() => wavexOsOnboardingApi.generateSwarm(selectedCompanyId, skipInference)}
          renderManifest={(data) => (
            <SwarmOrgChart
      ...
      {phase === "finalize" && (
        <GenerateManifestPhase
          ...
          generate={() => wavexOsOnboardingApi.complete({ companyId: selectedCompanyId, skipInference })}
          renderManifest={(data) => (
            <CompanyView
              data={data.manifest}
