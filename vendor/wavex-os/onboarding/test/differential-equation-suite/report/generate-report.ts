...
export async function aggregateReportData(): Promise<ReportData> {
  ...
  const notes: string[] = [];
  if (suite_1.total === 0) notes.push("Suite 1 has no records — run with WAVEX_OS_TEST_LIVE=1 to populate divergence data.");
  if (suite_4.total === 0) notes.push("Suite 4 has no records — run with WAVEX_OS_TEST_LIVE=1 to measure T2 value.");
    ...
    r.verdict === "SOLUTION_SURFACE_CONFIRMED"
      ...
      : r.verdict === "CURVE_DETECTED"
        ...
        : r.verdict === "PARTIAL_SURFACE"
          ...
          : "Not enough data yet. Run with WAVEX_OS_TEST_LIVE=1 to populate Suite 1 and Suite 4 records.",
  );
  return lines.join("\n") + "\n";
}
